#!/usr/bin/env python2
#
# This library was created for implementing faster and reliable network ranges, generating
# random ips, check for double targetips, randomize targetlist.
#
...
class targetlib(object):
    ...
    def randomizeIP(self, iplist):
        ...
        for ip in orig_list:
            if iplist.count(ip) > 1:
                printd('[*] Warning, %s is %d times in list' % (ip, iplist.count(ip)))
        ...
        while [1]:
            ...
            if cnt > epos * self.randiter:
                printd('[+] Did %d random iterations, break' % (self.randiter))
                break

        # check if all ips still there (you never know ;))
        for ip in orig_list:
            if iplist.count(ip) == 0:
                printd('Warning missing %s' % (ip))
    ...
    def _generate_ip(self):
        ...
        if blockOne == 10:
            ...
            newIp = str(blockOne) + '.' + str(blockTwo) + '.' + str(blockThree) + '.' + str(blockFour)
            return newIp
